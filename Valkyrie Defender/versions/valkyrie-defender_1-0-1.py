...
web_whitelist = {}
base_word_dic = {"gg":["Good game", "Buen juego / Bien jugado"], 
                 "btw":["By The Way", "Por cierto"], 
                 "ikr":["I Know, Right?", "Si, no?"], 
                 ...
                 "fwiw":["For What It's Worth", "Si sirve de algo"],
                 "iirc":["If I Remember Correctly", "Si recuerdo bien"],
                 "lmao":["Laughing My Ass Off", "Me cago de risa"],
                 "tldr":["Too Long; Didn't Read", "Mucho texto"],
                 "yolo":["You Only Live Once", "Solo se vive una vez"],
                 "afaik":["As Far As I Know", "Hasta donde sé"],
                 "brb":["Be Right Back", "Vuelvo enseguida"],
                 "wyd":["What are You Doing", "Que estás haciendo"],
...
# web whitelist
def load_web_whitelist():
    web_whitelist_file_path = os.path.join(script_directory, 'web_whitelist.txt')
    if not os.path.exists(web_whitelist_file_path):
        with open(web_whitelist_file_path, 'w') as f:
            json.dump([], f)
    with open(web_whitelist_file_path, 'r') as config_file:
        return json.load(config_file)

def save_web_whitelist(config):
    web_whitelist_file_path = os.path.join(script_directory, 'web_whitelist.txt')
    with open(web_whitelist_file_path, 'w') as config_file:
        json.dump(config, config_file)
...
def load_diccionario():
    diccionario_file_path = os.path.join(script_directory, 'diccionario.txt')
    if not os.path.exists(diccionario_file_path):
        with open(diccionario_file_path, 'w') as f:
            json.dump(base_word_dic, f)
    with open(diccionario_file_path, 'r') as config_file:
        return json.load(config_file)

def save_diccionario(config):
    diccionario_file_path = os.path.join(script_directory, 'diccionario.txt')
    with open(diccionario_file_path, 'w') as config_file:
        json.dump(config, config_file)
...
async def on_message(message):
    ...
    for url in urls:
        if "tenor.com" in url or "youtube.com" in url or "twitch.tv":
            isGif = True
            break
...
# add web to web_whitelist
@bot.command()
@commands.has_role('valkyrie_admin')
async def add_web(ctx, web):
    web_whitelist.append(str(web))
    save_web_whitelist(web_whitelist)
    await ctx.channel.send(f"Web '{web}' has added to the whitelist")

# show all webs in web_whitelist
@bot.command()
async def show_whitelisted_webs(ctx):
    ...
    if len(web_whitelist) > 0:
        ...
        await ctx.channel.send("There are no whitelisted webs.")

# remove web from web_whitelist
@bot.command()
@commands.has_role('valkyrie_admin')
async def remove_web(ctx, web):
    web_whitelist = load_web_whitelist()
    if web in web_whitelist:
        web_whitelist.remove(web)
        save_web_whitelist(web_whitelist)
        await ctx.channel.send(f"Web '{web}' has been removed from the whitelist.")
    else:
        await ctx.channel.send(f"Web '{web}' is not in the whitelist.")
        
# add word to word_dic, para lo de la definicion de palabras
@bot.command()
@commands.has_role('valkyrie_admin')
async def add_word(ctx, word, *args):
    meanings = ' '.join(args).split()
    if len(meanings) != 4 or meanings[0] != 'eng:' or meanings[2] != 'esp:':
        await ctx.send("The structure must be: *add_word word eng: meaning in english esp: meaning in spanish")
        return
    eng_meaning = meanings[1]
...
async def remove_word(ctx, word):
    if ctx.message.author.id == 666360540041445396:
        if word in word_dic:
            ...
            await ctx.channel.send(f"Word '{word}' is not in the dictionary.")
    else:
...
async def Help(ctx):
    await ctx.send(str("User guide: https://airilmusic.github.io/Valkyrie-Defender/#"+
                    ...
                    "\n\n*add_word eng: meaning in english esp: meaning in spanish"+
